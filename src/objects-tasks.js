...
function compareObjects(obj1, obj2) {
  const obj1Keys = Object.entries(obj1);
  const obj2Keys = Object.entries(obj2);
  if (obj1Keys.length !== obj2Keys.length) {
    return false;
  }
  let result = true;
  obj1Keys.forEach(([key, value]) => {
    if (obj2[key] !== value) {
      result = false;
    }
  });
...
function makeWord(lettersObject) {
  const maxLength = Object.values(lettersObject).flat().sort().slice(-1)[0];
  const arr = new Array(maxLength);
  Object.entries(lettersObject).forEach(([key, value]) => {
    value.forEach((keyOfVal) => {
      arr[keyOfVal] = key;
    });
  });
  return arr.join('');
...
function sellTickets(queue) {
  ...
  function action(index) {
    if (queue[index] === 25) {
      cashSeller[25] += 1;
      return index === queue.length - 1 ? true : action(index + 1);
    }
    if (queue[index] === 50 && cashSeller[25] !== 0) {
      cashSeller[25] -= 1;
      cashSeller[50] += 1;
      return index === queue.length - 1 ? true : action(index + 1);
    }
    if (queue[index] === 100) {
      const sum = cashSeller[25] * 25 + cashSeller[50] * 50;
  ...
  return queue.length === 0 ? true : action(0);
}
...
function group(array, keySelector, valueSelector) {
  const obj = {};
  array.forEach((countryObj) => {
    const key = keySelector(countryObj);
    const value = valueSelector(countryObj);
